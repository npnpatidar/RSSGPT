  ...
  if validate_entry(entry):
    ...
    cursor.execute(
        f'''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (entry.id, entry.title, entry.link, "NO SUMMARY", "",parser.parse(entry.published, dayfirst=True).strftime('%a, %d %b %Y %H:%M:%S %z'), False))
...
def update_summary_for_all_tables(pool, g4f):
  with concurrent.futures.ThreadPoolExecutor() as executor:
    ...
    for table_name in table_names:
      ...
      entries_with_no_summary = sorted(
          entries_with_no_summary,
          key=lambda x: datetime.strptime(x[4], '%a, %d %b %Y %H:%M:%S %z'),
          reverse=True)
      print(len(entries_with_no_summary), " in ", table_name)

      for entry_id, entry_url, entry_article_text, entry_title, entry_date in entries_with_no_summary:
        executor.submit(summarize_and_update, pool, g4f, table_name, entry_id,
                        entry_article_text, entry_title)
    pool.release_connection(connection)


def summarize_and_update(pool, g4f, table_name, entry_id, entry_article_text,
                         entry_title):
  try:
    connection = pool.get_connection()
    cursor = connection.cursor()
...
def update_article_text_for_all_tables(pool):
  with concurrent.futures.ThreadPoolExecutor() as executor:
    ...
    for table_name in table_names:
      ...
      print(len(entries_with_no_article_text), " in ", table_name)

      for entry_id, entry_url in entries_with_no_article_text:
        executor.submit(fetch_and_update_article_text, pool, table_name,
                        entry_id, entry_url)
