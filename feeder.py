...
def read_last_run_date(last_run_date_file):
  last_run_date = None
  try:
...
def write_last_run_date(last_run_date_file, date):
  with open(last_run_date_file, "w", encoding="utf-8") as file:
...
def simplify_string(s):
  ...
  s = re.sub(r'rss', '', s)  # Remove rss
  s = re.sub(r'feed', '', s)  # Remove feed
  s = re.sub(r'default', '', s)  # Remove default
...
def create_markdown_file(feed_url, entries):
  ...
  with open(file_name, "a", encoding="utf-8") as markdown_file:
    ...
    for date, date_entries in sorted(entries_by_date.items(), reverse=True):
      markdown_file.write(
          f"### {datetime(*date[:6]).strftime('%B %d, %Y')}\n\n")
      for entry in date_entries:
        ...
        print("summarising")
        generated_summary = summarise2(article_text)
        print("writing")
...
def summarise(article_text):
  ...
  response = g4f.ChatCompletion.create(
      ...
      max_tokens=
      500,  # Adjust the max_tokens to control the length of the summary
...
def summarise2(article_text):
  ...
  return "Failed to generate a valid summary after 10 attempts."
...
# Load the OPML file
#opml_file = "markdown_files/Feeds.opml"


def write_markdown_files(opml_file, last_run_date_file):
  ...
  for feed_url in feed_urls:
    ...
    if last_run_date:
      entries = [
          ...
          if datetime(*entry.published_parsed[:6]) > last_run_date
      ]
  ...
  write_last_run_date(last_run_date_file, datetime.now())
